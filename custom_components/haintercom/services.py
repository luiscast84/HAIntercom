...
from homeassistant.const import ATTR_ENTITY_ID, ATTR_TARGET

from .const import DOMAIN, SERVICE_START_INTERCOM, SERVICE_STOP_INTERCOM, SERVICE_SEND_MESSAGE

SCHEMA_START_INTERCOM = vol.Schema(vol.All(
...
SCHEMA_STOP_INTERCOM = vol.Schema(vol.All(
    cv.make_entity_service_schema({})
))

SCHEMA_SEND_MESSAGE = vol.Schema(vol.All(
    cv.make_entity_service_schema(
        {
...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def handle_start_intercom(call: ServiceCall) -> None:
        ...
        target_entity_id = call.data.get("entity_id", [call.target["entity_id"][0]])[0]
